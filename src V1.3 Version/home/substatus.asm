...
HandleDamageReductionExceptSubstatus2::
	...
	jr z, .no_damage
	cp SUBSTATUS1_2XDAMAGE_TO_SELF ; This new effect doubles damage to self in opp's next turn.
...
; when Magcargo is damaged, if its Strikes Back is active, the
...
HandleNShieldAndTransparency::
	...
	jr z, .nshield
	cp PROFESSOR_ELM
	jr z, .transparency ;REMOVED FROM GAME EFFECTIVELY, see below
.done
	pop de
	or a
	ret
...
HandleTransparency::
	ld a, [wTempNonTurnDuelistCardID]
	cp PROFESSOR_ELM
	jr z, .done ; Altered to done so that transparency is effectively removed
.done
	or a
	ret
.transparency
...
; if the id of the card provided in register a as a deck index is FERALIGATR2,
