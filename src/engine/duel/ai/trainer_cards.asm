...
AIDecide_SuperPotion1:
	...
	ld e, PLAY_AREA_ARENA
	call GetCardDamageAndMaxHP
	cp 60 + 1 ; if damage < 40, changed to 60 for my hack
	jr c, .calculate_hp
	ld a, 60
.calculate_hp
	ld l, a
...
; go to the routines associated with deck ID
	...
	jr z, .pikachu
	cp ETCETERA_DECK_ID ; SAME
	jr z, .etcetera
	cp LOVELY_NIDORAN_DECK_ID ;SAME
	jp z, .lovely_nidoran
	or a
	ret
...
; skip if number of cards in hand < 3
	...
	ld a, [wOpponentDeckID]
	cp LEGENDARY_RONALD_DECK_ID ; CHANGE TO RONALD FINALE
	...
	cp LEGENDARY_DRAGONITE_DECK_ID ; LEGENDARY DRGAONITE
...
; reaching here is extremely unlikely. ; NOTE TO SELF: IT DID HAPPEN ONCE
