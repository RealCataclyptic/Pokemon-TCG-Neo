...
CheckAbleToRetreat:
	call CheckCantRetreatDueToAcid ; checks acid effect
	ret c
	call CheckIfActiveCardParalyzedOrAsleep
	ret c
	call CheckCantRetreatDueToAbility ; checks substatus ability for no retreat
	ret c
...
.choose_arena_loop
	...
	ldh a, [hTempCardIndex_ff98]
	call LoadCardDataToBuffer1_FromDeckIndex
	ld a, PRACTICEDUEL_PLAY_MANTINE
	call DoPracticeDuelAction
	jr c, .choose_arena_loop
	ldh a, [hTempCardIndex_ff98]
	call PutHandPokemonCardInPlayArea
...
.choose_bench
	...
	ld a, PRACTICEDUEL_PUT_SEADRA_IN_BENCH
	call DoPracticeDuelAction
.bench_loop
	ld a, TRUE
...
; FOSSIL_EGG and POKE_DOLL do count as basic Pokemon cards
...
; return nc if the card at wLoadedCard1 is a basic Pokemon card
; FOSSIL_EGG and POKE_DOLL do NOT count unless already checked
...
.skip_status
	...
	ld a, [wCurPlayAreaSlot]
	add DUELVARS_ARENA_CARD_ATTACHED_STRENGTH_CHARM
	call GetTurnDuelistVariable
	or a
	jr z, .not_pluspower
	ld a, [wCurPlayAreaY]
...
; discard any STRENGTH CHARM attached to the turn holder's arena and/or bench Pokemon
...
; discard any CRYSTAL SHARD attached to the turn holder's arena and/or bench Pokemon
...
HandleSleepCheck:
	...
	jr nc, .not_GengarOrScareActive
	or a
	ldtx de, DeepSleepText ; Deep Sleep Text
	call TossCoin
	ld a, DUEL_ANIM_SLEEP
	ldtx hl, IsStillAsleepText
	jr nc, .tails

.not_GengarOrScareActive
...
; prompt the player to replace the knocked out Pokemon with one from bench
	...
	ld a, $01
	ld [wcbd4], a
	ld a, PRACTICEDUEL_PLAY_SEADRA_FROM_BENCH
	call DoPracticeDuelAction
.select_pokemon
	call OpenPlayAreaScreenForSelection
	jr c, .select_pokemon
...
.ConductivityScenario
	...
	call SwapTurn
	jr nc, .Return ; if not the right mon, ignore.
	...
	call Func_6e4c ; nessesary if the damage KO's the mon, calls for the knocked out process.
	ret ; just in case because I'm paranoid.
