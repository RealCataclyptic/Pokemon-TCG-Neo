...
FeintAttackDamageEffect:
	...
	ld b, a
	ld de, 40
	call DealDamageToPlayAreaPokemon_RegularAnim ; Special animation found below to make it look good.
	call SwapTurn
	ret
...
DracoMeteorDamageEffect:
	...
	ld b, a
	ld de, 80
	call DealDamageToPlayAreaPokemon_DracoAnim ; Special animation found below to make it look good.
	call SwapTurn
	ret

DealDamageToPlayAreaPokemon_DracoAnim:
...
DealDamageToPlayAreaPokemon_PsyAnim:
	ld a, ATK_ANIM_SCREEN_PSYCHIC_HIT 
	ld [wLoadedAttackAnimation], a
	jp DealDamageToPlayAreaPokemon	

DealDamageToPlayAreaPokemon_PsysplashAnim:
	ld a, ATK_ANIM_PSYSPLASH 
	ld [wLoadedAttackAnimation], a
	jp DealDamageToPlayAreaPokemon
...
Deal10DamageToTargetPerEnergy_DamageEffect:
	...
	ldh a, [hTempPlayAreaLocation_ffa1]
	ld b, a
	call DealDamageToPlayAreaPokemon_PsyAnim
	call SwapTurn
	ret
...
MoreDamageifDFPDamagedAIEffect:
	call MoreDamageifDFPDamagedEffect
	jp SetDefiniteAIDamage

SpecialParalysisEffect:
	farcall SpecialParalysisEffect2
	ret
...
BoostDamageifLowHPAIEffect:
	call BoostDamageifLowHPEffect
	jp SetDefiniteAIDamage
	

SLPHealEffect:
	lb de, 0, 30 ; heal 30 damage
	call ApplyAndAnimateHPRecovery
	call SwapTurn
...
NidorinaDoubleKick_MultiplierEffect:
	farcall NidorinaDoubleKick_MultiplierEffect2
	ret

NidorinoDoubleKick_AIEffect:
	farcall NidorinoDoubleKick_AIEffect2
	jp SetExpectedAIDamage
...
FossilEggSearch_PlayerSelectEffect:
	...
	ldtx bc, KrabbyText
	lb de, SEARCHEFFECT_CARD_ID, FOSSIL_EGG
	call LookForCardsInDeck
	ret c
...
PoliwagWaterGunEffect: ; Is now Heavy Slam code
	call SwapTurn
	ld e, PLAY_AREA_ARENA
	call GetPlayAreaCardRetreatCost
	call ATimes10
...
FocusEnergyEffect:
	...
	or a
	ret z ; return if no damage dealt
	ld a, SUBSTATUS1_2XDAMAGE_TO_SELF
	call ApplySubstatus1ToDefendingCard
	ret
...
RaichuLS_DamageEffect:
	...
	ld a, ATK_ANIM_THUNDER_WHOLE_SCREEN
	ld [wLoadedAttackAnimation], a
	ld a, 80
...
PlayerYesNoFlareonEffect:
	bank1call DrawDuelMainScene
	ldtx hl, FlareonYesNoText
	call TwoItemHorizontalMenu
	ldh a, [hKeysHeld]
	and B_BUTTON
	ldh a, [hCurMenuItem]
	jr nz, PlayerYesNoFlareonEffect
	ldh a, [hCurMenuItem]
	ldh [hTemp_ffa0], a
	ret
FlareonDamageEffect:
...
PlayerForLSelectionHoohCEffect:
	farcall PlayerForLSelectionHoohCEffect2
	ret

HoohSelectedFEffect:
	farcall HoohSelectedFEffect2
	ret

HoohRecoverEffect:
	farcall HoohRecoverEffect2
	ret

Hooh_AISelectEffect:
	farcall Hooh_AISelectEffect2
	ret

HoohMoreDamageEffect:
	farcall HoohMoreDamageEffect2
	ret
...
FinalBlowEffect:
	farcall FinalBlowEffect2
	ret

FinalBlowAIEffect:
	farcall FinalBlowAIEffect2
	ret

FinalBlowPPEffect: ; just to keep the game from messing up
	scf
	ret
