SuperScoopUpDescription: 
	text "Choose 1 of your Pokémon in play"
	line "and return its Basic Pokémon card to"
	line "your hand. (Discard all cards"
	line "attached to that card.)"
	done

PokegearName: 
	text "PokéGear"
	done

PokegearDescription: 
	text "Discard 2 of the other cards from"
	line "your hand in order to search your"
	line "deck for any card and put it into"
	line "your hand. Shuffle your deck"
	line "afterward."
	done

NewPokedexName: 
	text "New Pokédex"
	done

NewPokedexDescription: 
	text "Look at the top 5 cards of your"
	line "deck and rearrange them as you"
	line "like."
	done

StrengthCharmName: 
	text "Strength Charm"
	done

StrengthCharmDescription: 
	text "Attach Strength Charm to your Active"
	line "Pokémon; At the end of your turn,"
	line "discard it. If this Pokémon's attack"
	line "deals damage to the Active Pokémon,"
	line "it does 10 more damage. (After"
	line "applying Weakness and Resistance)"
	done

CrystalShardName: 
	text "Crystal Shard"
	done

CrystalShardDescription: 
	text "Attach Crystal Shard to your Active"
	line "Pokémon; At the end of your"
	line "opponent's next turn, discard it."
	line "Damage done to that Pokémon is"
	line "reduced by 20 (after applying"
	line "Weakness and Resistance)."
	done

RuinWallName: 
	text "Ruin Wall"
	done

RuinWallDescription: 
	text "Discard 2 of the other cards from"
	line "your hand in order to put a Trainer"
	line "card from your discard pile into"
	line "your hand."
	done

PokemonReversalName: 
	text "Pokémon Reversal"
	done

PokemonReversalDescription: 
	text "Choose 1 of your opponent's Benched"
	line "Pokémon and switch it with his or"
	line "her Active Pokémon."
	done

HyperDevolutionSprayName: 
	text "Hyper Devolution Spray"
	done

HyperDevolutionSprayDescription: 
	text "Choose 1 of your own Pokémon in play"
	line "and a Stage of Evolution. Discard"
	line "all Evolution cards of that Stage or"
	line "higher attached to that Pokémon."
	done

HyperDevolutionSprayDescriptionCont: 
	text "That Pokémon is no longer Asleep,"
	line "Confused, Paralyzed, Poisoned, or"
	line "anything else that might be the"
	line "result of an attack (just as if you"
	line "had evolved it)."
	done

PotionName:
	text "Potion"
	done

PotionDescription: 
	text "Remove 3 damage counters from 1 of"
	line "your Pokémon. If that Pokémon has"
	line "fewer damage counters than that,"
	line "remove all of them."
	done

HyperPotionName: 
	text "Hyper Potion"
	done

HyperPotionDescription:
	text "Discard 1 Energy card attached to 1"
	line "of your own Pokémon in order to"
	line "remove 6 damage counters from that"
	line "Pokémon. If the Pokémon has fewer"
	line "damage counters than that, remove"
	line "all of them."
	done

FullHealName:
	text "Full Heal"
	done

FullHealDescription: 
	text "Your Active Pokémon is no longer"
	line "Asleep, Confused, Paralyzed, or"
	line "Poisoned. Then, draw 1 card."
	done

MiracleBerryName: 
	text "Miracle Berry"
	done

MiracleBerryDescription: 
	text "Put 1 Basic Pokémon card from your"
	line "discard pile onto your Bench."
	line "Put damage counters on that Pokémon"
	line "equal to half its HP (rounded down"
	line "to the nearest 10). (You can't play"
	line "this card if your Bench is full.)"
	done

BillsMaintenanceName: 
	text "Bill's Maintenance"
	done

BillsMaintenanceDescription: 
	text "Shuffle 2 of the other cards from"
	line "your hand into your deck in order"
	line "to draw 2 cards."
	done

PokemonFluteChannelName: 
	text "Pokémon Flute Channel"
	done

PokemonFluteChannelDescription:
	text "Choose 1 Basic Pokémon card from"
	line "your opponent's discard pile and put"
	line "it onto his or her Bench. (You can't"
	line "play this card if your opponent's"
	line "Bench is full.)"
	done

ArcadeGameName:
	text "Arcade Game"
	done

ArcadeGameDescription:
	text "Shuffle your hand into your deck."
	line "Flip a coin. If heads, draw 9 cards."
	line "If tails, draw 1 card."
	done

OldRodName: 
	text "Old Rod"
	done

OldRodDescription: 
	text "Flip a coin. If heads, put a card"
	line "in your discard pile on top of your"
	line "deck."
	done
