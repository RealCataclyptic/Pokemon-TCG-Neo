...
; choose to randomly return
	...
	ld a, DUELVARS_NUMBER_OF_POKEMON_IN_PLAY_AREA  ; energy trans is not in legacy
...
; check heal
	cp VILEPLUME ; replaced with Poison Pollen Ai Effects
	jr nz, .check_peek
	call HandleAIHeal
	jr .next_1
.check_shift ; Shift doesn't work AT ALL for some reason (even if you ovveride handleAIshift below), deleted.
	cp MAIL_FROM_BILL
	jr nz, .check_peek
	call HandleAIShift
	jr .next_1
...
.check_strange_behavior
	cp MAIL_FROM_BILL ; deleted from legacy
	jr nz, .check_curse
...
HandleAIPeek:
	ld a, 10
	call Random
	cp 4 ; 4 in 10 chance of using it
	jr c, .UseDriveOff
	ret
...
.UseDriveOff
	...
	bank1call AIMakeDecision
	pop af
	farcall PidgeottoWhirlwind_SelectEffect
...
HandleAIDamageSwap:
	...
	ld a, PROFESSOR_ELM
	call CountPokemonIDInPlayArea
	ret nc ; return if not slowking, deleted from legacy
	ld a, FERALIGATR2
	call CountPokemonIDInBothPlayAreas
	ret c ; return if there's Muk in play
