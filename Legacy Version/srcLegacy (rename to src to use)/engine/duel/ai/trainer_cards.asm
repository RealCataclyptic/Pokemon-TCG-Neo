...
; move on to the next item in list.
	...
	jp c, .next_in_data
	bank1call CheckMindGamesScenario
	jp c, .next_in_data
...
AIPlay_Pluspower:
	ld a, [wCurrentAIFlags]
	or AI_FLAG_USED_STRENGTH_CHARM
	ld [wCurrentAIFlags], a
	ld a, [wAITrainerCardParameter]
...
AIPlay_GustOfWind:
	ld a, [wCurrentAIFlags]
	or AI_FLAG_USED_POKEMON_REVERSAL
	ld [wCurrentAIFlags], a
	ld a, [wAITrainerCardToPlay]
	ldh [hTempCardIndex_ff9f], a
	ld a, [wAITrainerCardParameter]
...
; do not use it again.
	ld a, [wPreviousAIFlags]
	and AI_FLAG_USED_POKEMON_REVERSAL
	ret nz
...
AIPlay_SuperEnergyRemoval: ; This is now Master Ball in the hack
	ld a, [wAITrainerCardToPlay]
	ldh [hTempCardIndex_ff9f], a
	ld a, [wAITrainerCardParameter]
	ldh [hTempPlayAreaLocation_ffa1], a
...
; stage 2 Pokemon cards ; Doing ALL Stage 2's for now, however, adjust list AFTER Star Pieces have been distributed.
	ld d, a
	call LoadCardDataToBuffer1_FromDeckIndex
	cp MEGANIUM1
	jr z, .found
...
AIPlay_ProfessorOak:
	ld a, [wCurrentAIFlags]
	or AI_FLAG_USED_PROFESSOR_ELM | AI_FLAG_MODIFIED_HAND
	ld [wCurrentAIFlags], a
	ld a, [wAITrainerCardToPlay]
	ldh [hTempCardIndex_ff9f], a
	ld a, OPPACTION_EXECUTE_TRAINER_EFFECTS
...
; handle some decks differently
	ld a, [wOpponentDeckID]
	cp STRANGE_PSYSHOCK_DECK_ID ; changed to Psychic Club leader
...
.imakuni
; has a 2 in 10 chance of returning carry ; Edited to 4 for TCG neo
	ld a, 10
	call Random
	cp 4
	jr nc, .no_carry
...
; go to the routines associated with deck ID
	ld a, [wOpponentDeckID]
	cp FLOWER_GARDEN_DECK_ID ; NOW IT IS FLOWERGARDEN
	jr z, .fire_charge
	cp HARD_POKEMON_DECK_ID ; SAME
	jr z, .hard_pokemon
...
; skip if number of cards in hand < 3
	...
	ld a, [wOpponentDeckID]
	cp LEGENDARY_RONALD_DECK_ID ; CHANGED TO RONALD FINALE
	jr z, AIDecide_ComputerSearch_RockCrusher
	cp WONDERS_OF_SCIENCE_DECK_ID ; KEEP SAME
	jp z, AIDecide_ComputerSearch_WondersOfScience
	cp FIRE_CHARGE_DECK_ID ; SAME
	jp z, AIDecide_ComputerSearch_FireCharge ; 
	cp LEGENDARY_DRAGONITE_DECK_ID ; LEGENDARY DRAGONITE
...
; what Pokemon to look for.
	ld a, [wOpponentDeckID]
	cp POWERFUL_RONALD_DECK_ID 
	jr z, AIDecide_PokemonTrader_LegendaryMoltres ; THIS DECK ID IS SIMPLE
	cp STRANGE_PSYSHOCK_DECK_ID ;
	jr z, AIDecide_PokemonTrader_LegendaryArticuno ; LOOKS FOR TWO CARDS AND A 2 STAGE EVO
	cp ZAPPING_SELFDESTRUCT_DECK_ID
	jp z, AIDecide_PokemonTrader_LegendaryDragonite ; LOOKS FOR MANY EVOS
	cp GO_GO_RAIN_DANCE_DECK_ID
	jp z, AIDecide_PokemonTrader_LegendaryRonald ; LOOKS FOR MANY EVOS
	cp ROCK_CRUSHER_DECK_ID
	jp z, AIDecide_PokemonTrader_BlisteringPokemon ; LOOKS FOR 3 2-STAGE EVOS
	cp SOUND_OF_THE_WAVES_DECK_ID
	jp z, AIDecide_PokemonTrader_SoundOfTheWaves ; LOOKS FOR 5 2-STAGE EVOS, 
	cp FLOWER_POWER_DECK_ID
	jp z, AIDecide_PokemonTrader_PowerGenerator ; LOOKS FOR MANY EVOS
	cp WONDERS_OF_SCIENCE_DECK_ID
	jp z, AIDecide_PokemonTrader_FlowerGarden ; LOOKS FOR MANY EVOS
	cp FIRST_STRIKE_DECK_ID ; //// DONE, ONLY LOOKS FOR AERODACTYL, 
	jp z, AIDecide_PokemonTrader_StrangePower ; DECK ID IS SIMPLE///DONE
	cp FIRE_CHARGE_DECK_ID
	jp z, AIDecide_PokemonTrader_Flamethrower ; LOOKS FOR MANY EVOS
	or a
	ret

AIDecide_PokemonTrader_LegendaryMoltres:
;UNKNOWN DECK DECIDE LATER
...
AIDecide_PokemonTrader_LegendaryDragonite: ; Unused for now
...
; hand and if so, choose it as target instead.
	...
	jr c, .find_duplicates	
	jr .no_carry
	; bug, missing jr .no_carry ; CORRECTED
